...
export class ProjectListComponent implements OnInit {
  ...
  count(): void {
    this.projectService.count().subscribe((result) => {
      console.log(result);
  ...
  loadPage(pg: number) {
    this.currentPage = pg;
    this.projectService.list(pg, this.limit).subscribe((result) => {
      console.log(result);
  ...
  delete(project: Project): void {
    Swal.fire({
      title: '¿Está seguro?',
      text: `El registro de id ${project.idproject} será eliminado permanentemente`,
      icon: 'warning',
